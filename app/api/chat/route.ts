...
import { searchMultiQuery, deduplicateAcrossQueries } from "@/lib/tools/web-search";
import { scrapeUrls } from "@/lib/tools/web-scrape";
import { executeCode } from "@/lib/tools/code-execute";
import { extremeSearch } from "@/lib/tools/extreme-search";
...
function getActiveTools(mode: SearchMode) {
  if (mode === "extreme") {
    // Only expose extremeSearch — it handles searching/scraping/code internally
...
export async function POST(req: Request) {
  ...
  const allTools = {
    webSearch: tool({
      ...
      execute: async ({ queries }) => {
        ...
        // Auto-scrape top results to enrich content (SearXNG snippets are too short)
  ...
  const result = streamText({
    ...
    // Auto-fix malformed tool calls from the model
    ...
    // Disable parallel tool calls — run one at a time for better flow control
    providerOptions: {
